...
object Motor extends App {
  ...
  val mainMenuMouseListener: MouseAdapter = new MouseAdapter {
    override def mouseClicked(e: MouseEvent): Unit = {
      ...
      if (hostButton.contains(x, y) || joinButton.contains(x, y) || exitButton.contains(x, y)) {
  ...
  // Prevent the other player being still in the game when one has shutdown its app
  ...
  // Constantly refresh the GUI in a parrallel thread
  Future {
    while (true) {
      if (pageId == 4)
  ...
  def initGame(): Unit = {
    ...
    room.movePlayer(room.getPlayer(1),
      1 + scala.util.Random.nextInt(5),
      1 + scala.util.Random.nextInt(13)
    )

    room.movePlayer(room.getPlayer(2),
      13 + scala.util.Random.nextInt(5),
      1 + scala.util.Random.nextInt(13)
    )
  ...
  /**
   * Connect to the given IP and start the communication and game
   *
  ...
  def connectTo(ip: String): Unit = {
    ...
    // Block the main thread before starting the game
  ...
  /**
   * Display the page components based on the id value
   */
  ...
  /**
   * Display the components for the menu page
   */
  ...
  /**
   * Display the components for the host page
   */
  ...
  /**
   * Display the components for the client page
   */
